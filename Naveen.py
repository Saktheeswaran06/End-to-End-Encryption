...
def naveen(shared_key):
    host = 'localhost'
    port = 12345

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect((host, port))
    print("Naveen: Connected to server")
    ...
    while True:
        ...
        print("Naveen: Received Hash:", binascii.hexlify(received_hmac))  # Print the received HMAC
        # Decrypt message
        decrypted_message = decrypt_message(encrypted_message, shared_key, iv_sakthi)
        print("Naveen: Decrypted message from Sakthi:", decrypted_message)
        ...
        if computed_hmac == received_hmac:
            print("Naveen: Hash verification successful.")
        else:
            print("Naveen: Hash verification failed. Message integrity compromised.")

        response = input("Naveen: Enter the message to send to Sakthi (type 'exit' to quit): ")
        if response == 'exit':
            break

        iv = os.urandom(16)  # Generate random IV (Initialization Vector)
        encrypted_response = encrypt_message(response, shared_key, iv)
        print("Naveen: IV:", binascii.hexlify(iv))
        print("Naveen: Encrypted response:", binascii.hexlify(encrypted_response))

        # Generate HMAC for the response
        hmac_digest = generate_hmac(response.encode(), shared_key)

        print("Naveen: Hash:", binascii.hexlify(hmac_digest))
        
        # Send IV, encrypted message, and HMAC to the server
        s.sendall(iv + encrypted_response + hmac_digest)  
        print("Naveen: Encrypted response and Hash sent to the server.")
